...
| 100 | 0x00000064 | CallStatus JSON (phone call state) | 17-130 bytes |
...
```json
{
  "CallStatus": 2,
  "CallDirection": 1,
  "CallID": "071CF459-79CB-475B-98DF-0F10CB655A11",
  "CallName": "Jenny",
...
|-------|------|-------------|
| `CallStatus` | int | 0=idle/ended, 1=dialing, 2=ringing, 3=connected, 4=disconnecting |
...
**State machine (verified all 4 scenarios):**
```
         Outgoing:  0 → 1 (dialing) → 4 (disconnecting) → 0 (idle)
  Incoming answer:  0 → 2 (ringing) → 3 (connected) → 4 (disconnecting) → 0
  Incoming reject:  0 → 2 (ringing) → 0 (idle, skips 3/4)
Incoming voicemail: 0 → 2 (ringing) → 0 (idle, skips 3/4, after ~20s ring)
```
...
| 0x2A | DashBoard_DATA | Variable | Parse subtype: 1=media JSON, 3=album art, 100=CallStatus JSON, 200=NaviJSON, 201=NAVI_IMAGE (AA-only PNG maneuver icon). Update media session, call state, and navigation state. |
